                    ...
                    if sample_file.endswith('.npz'):
                        ...
                        for sample_index in range(len(possible_sample_pcl)):
                            ...
                            if len(visible_sample) == 0:
                                ...
                                print(f'It is visible {GREEN}{len(visible_sample)}{DEFAULT} points from {name_of_object}. Moving to another scene')
                                match_find = True
                                ...
                                create_annotation(old_address=f'{DATA_PATH}/label_2/{frame_name}.txt',
                                                  new_address=f'{SAVE_OUTPUT_FOLDER}/{save_folder}/label_2/{frame_name}.txt',
                                                  additional_line=sample_annotation)

                                ## SAVING PCL TO .BIN
                                save_file = open(f'{SAVE_OUTPUT_FOLDER}/{save_folder}/data/{frame_name}.bin', 'wb')
