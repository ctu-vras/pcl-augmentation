...
class SemanticKITTI():
    ...
    def __getitem__(self, idx):
        ...
        return pcl, transform_matrix, f'{self.anno_path}/sequences/{self.sequence}/bbox/{frame_name}.txt', instance, self.sequence
...
class Waymo():
    def __init__(self, config):
        ...
        self.LiDAR_location = np.array([1.22, 0, 2])
    ...
    def __getitem__(self, idx):
        ...
        pcl[:, 0:3] -= self.LiDAR_location

        transform_matrix = np.load(matrix_file).reshape(4, 4)

        correction_matrix = np.eye(4)
        correction_matrix[0:3, 3] = self.LiDAR_location.T

        return pcl, transform_matrix @ correction_matrix, f'{self.anno_path}/{sequence}/bbox/{frame_name}.txt', instances, sequence

    def delete_item(self, idx, subdirectoties=True):
        self.velodyne_list = np.delete(self.velodyne_list, idx)
        ...
        if len(self.velodyne_list) == 0:
            ...
            if self.sequence != self.velodyne_list[0].split('/')[-3]:

                if subdirectoties:

                    self.create_subdirectories(self.velodyne_list[0].split('/')[-3])

            self.sequence = self.velodyne_list[0].split('/')[-3]

    def save_data(self, point_cloud, added_points, folder, name, idx):
        point_cloud[:, 0:3] += self.LiDAR_location
        added_points[:, 0:3] += self.LiDAR_location
        point_cloud, pcl_labels = self.remove_space_for_spherical(point_cloud)
        added_points, added_labels = self.remove_space_for_spherical(added_points)
        added_points = np.hstack((added_points, added_labels))
    ...
    def create_directories(self, save_folder):
        ...
        self.create_subdirectories(self.sequence)
