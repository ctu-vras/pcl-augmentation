...
def make_dictionary(annotation_array):
    ...
    annotation_dictionary = {'center': center, 'rotation': rotation, 'length': annotation_array[2][0],
                             'width': annotation_array[2][1], 'height': annotation_array[2][2], 'class': annotation_array[3][0]}
...
def correct_height(scene_pcl, sample_pcl, sample_anno, config):
    """
    ...
    while len(surface) == 0:
        ...
        surface = surface[surface[:, 4] == config['labels']['Road']]
...
def find_possible_places(point_cloud, scene_annotation, sample_data, map_data, original_pcl, config):
    """
    ...
    for rot in range(1, 361):
        ...
        if on_road:
            sample_pcl, sample_annotation, near_road = correct_height(original_pcl, sample_pcl, sample_annotation, config)
            if not near_road:
                not_on_road += 1
                continue
